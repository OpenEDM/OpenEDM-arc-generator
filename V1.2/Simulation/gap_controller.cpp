...
extern "C" __declspec(dllexport) void gap_controller(struct sCONTROLLER **opaque, double t, union uData *data)
{
   ...
   bool arc_current_rising = I_ARC > instance->arc_current_previous;
   instance->arc_current_previous = I_ARC;
   ...
   switch(instance->state) {
      ...
      case IGNITION:
         ...
         break;
      case ARC:
         // Abort arc when arc current is not rising anymore and falls below min arc current.
         if (!arc_current_rising && I_ARC < I_arc_min) {
            instance->state = IDLE;
         }
         break;
   }
