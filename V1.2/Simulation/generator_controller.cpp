...
extern "C" __declspec(dllexport) void generator_controller(struct sCONTROLLER **opaque, double t, union uData *data)
{
   ...
   bool   &Q1_CTRL     = data[6].b; // output
   bool   &Q2_CTRL     = data[7].b; // output
   ...
   // Disable charging (Q1_CTRL = false) for the current period if either:
   ...
   // Q1_CTRL follows the persistent `charging` flag.
   Q1_CTRL = instance->charging;

   // Q2_CTRL is true outside the centered `t_on` window around T/2.
   // I.e., Q2_CTRL is true when t_rel is inside any ot these two intervals:
   // 1. (0, T/2 - t_on/2).
   // 2. (T/2 + t_on/2, T).
   Q2_CTRL = (t_rel < (T / 2 - t_on / 2)) || (t_rel > (T / 2 + t_on / 2));
}
